package com.evan.lustre.service.base;
...
public class AbstractCrudService<DOMAIN, ID> implements CrudService<DOMAIN, ID> {
     ...
     * @param id id
     * @return DOMAIN
     * @throws NotFoundExceptionAbstract If the specified id does not exist
     */
    @NonNull
    @Override
    public DOMAIN getById(@NonNull ID id) {
        return fetchById(id).orElseThrow(() -> new NotFoundExceptionAbstract(domainName + " was not found or has been deleted"));
    }
    ...
    public void mustExistById(@NonNull ID id) {
        if (!existsById(id)) {
            throw new NotFoundExceptionAbstract(domainName + " was not exist");
        }
    }
